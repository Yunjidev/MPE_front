  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pendingEnterprisesData = await getData("admin/enterprises/not-validate");
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      <div className="p-4 bg-white rounded-lg shadow-md dark:bg-neutral-800">
        ...
        <p className="mt-2 text-2xl font-bold text-gray-900 dark:text-gray-50">{pendingEnterprises}</p>
      </div>
      <div className="p-4 bg-white rounded-lg shadow-md dark:bg-neutral-800">
        <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-100">Validated Companies</h2>
        <p className="mt-2 text-2xl font-bold text-gray-900 dark:text-gray-50">{validatedEnterprises}</p>
      </div>
