      ...
      <div className="fixed top-0 left-1/2 transform -translate-x-1/2 w-full max-w-full z-50 mt-4">
        <div className="navbar flex justify-between items-center backdrop-filter backdrop-blur-lg bg-[#F8D7DA] bg-opacity-30 dark:bg-[#232323] dark:bg-opacity-30 dark:text-white border border-neutral-200 font-semibold mx-auto w-5/6 h-16 rounded-xl neon-nav">
          <div className="flex items-center">
            <Link to="/">
              <img src={logo} alt="logo" className="w-10 h-10 ml-2 lg:ml-9 transition-transform duration-300 hover:scale-110" />
            </Link>
          </div>
          <div className="hidden md:flex justify-center gap-8 text-black dark:text-white">
            {userType === 'client' ? (
              <>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden backdrop-filter backdrop-blur-lg bg-[#F8D7DA] bg-opacity-30 dark:bg-[#232323] dark:bg-opacity-30 dark:text-white border border-neutral-200 font-semibold mx-auto w-5/6 mt-4 rounded-xl neon-nav">
            <div className="flex flex-col items-center gap-4 py-4">
              {userType === 'client' ? (
                <>
                  <Link to="/home-client" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>Accueil</Link>
                  <a href="#" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>Recherche</a>
                  <Link to="/faq" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>FAQ</Link>
                  <Link to="/about" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>À Propos</Link>
                </>
              ) : userType === 'enterprise' ? (
                <>
                  <Link to="/home-enterprise" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>Accueil</Link>
                  <a href="#" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>Recherche</a>
                  <Link to="/pricing" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>Tarifs</Link>
                  <Link to="/faq" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>FAQ</Link>
                  <Link to="/about" onClick={toggleMobileMenu} className='hover:text-neutral-600 hover:dark:text-neutral-300'>À Propos</Link>
                </>
              ) : null}
